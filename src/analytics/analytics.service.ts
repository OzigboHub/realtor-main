...
export class AnalyticsService {
  ...
  async getAgentAnalytics(agentId: string) {
    ...
    const conversionRate =
      totalInquiries > 0 ? (completedAppointments / totalInquiries) * 100 : 0;
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const simulatedViews =
        Math.floor(totalViews / 6) + Math.floor(Math.random() * 50);
  ...
  async getLandlordLedger(landlordId: string) {
      ...
      b.units.forEach((u) => {
        u.leases.forEach((l) => {
          ...
          l.rentPayments.forEach((rp) => {
            ...
            if (rp.status === 'PENDING' || rp.status === 'OVERDUE')
              totalPendingArrears += rp.amount;
          });
        });
    ...
    const monthlyBreakdown = [
      {
        month: 'Jan',
        revenue: totalPaidRevenue * 0.15,
        expenses: totalMaintenanceExpenses * 0.1,
      },
      {
        month: 'Feb',
        revenue: totalPaidRevenue * 0.18,
        expenses: totalMaintenanceExpenses * 0.15,
      },
      {
        month: 'Mar',
        revenue: totalPaidRevenue * 0.16,
        expenses: totalMaintenanceExpenses * 0.2,
      },
      {
        month: 'Apr',
        revenue: totalPaidRevenue * 0.17,
        expenses: totalMaintenanceExpenses * 0.18,
      },
      {
        month: 'May',
        revenue: totalPaidRevenue * 0.14,
        expenses: totalMaintenanceExpenses * 0.17,
      },
      {
        month: 'Jun',
        revenue: totalPaidRevenue * 0.2,
        expenses: totalMaintenanceExpenses * 0.2,
      },
    ];
